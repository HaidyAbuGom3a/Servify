...
class VerifyCodeViewModel @Inject constructor(
    ...
    BaseViewModel<VerifyCodeUiState, VerifyCodeUiEffect>(VerifyCodeUiState()),
    VerifyCodeInteractionListener {

    private val args = VerifyCodeArgs(savedStateHandle)
    ...
    override fun onClickResetCode() {
        _state.update { it.copy(isLoading = true) }
        tryToExecute(
            { manageAuth.sendOtpToEmail(args.email) },
            ::onResendCodeSuccess,
            ::onError
        )
    }
    ...
    override fun onClickConfirm() {
        ...
        tryToExecute(
            { manageAuth.confirmOtp(state.value.otp) },
            ::onConfirmOtpSuccess,
            ::onError
        )
    }

    private fun onConfirmOtpSuccess(unit: Unit) {
        _state.update { it.copy(isLoading = false) }
        sendNewEffect(VerifyCodeUiEffect.NavigateToResetPassword(state.value.otp))
    }
    ...
    override fun onClickBackIcon() {
        sendNewEffect(VerifyCodeUiEffect.NavigateUp)
    }
